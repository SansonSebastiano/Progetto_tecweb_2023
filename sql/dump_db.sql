...
CREATE TABLE `articolo` (
  ...
  `luogo` varchar(255) COLLATE utf8mb4_unicode_ci DEFAULT NULL,
  `descrizione` varchar(255) COLLATE utf8mb4_unicode_ci NOT NULL,
  `contenuto` varchar(2000) COLLATE utf8mb4_unicode_ci NOT NULL,
  `image_path` varchar(1024) COLLATE utf8mb4_unicode_ci DEFAULT NULL,
  `tag` enum('scoperta','new-entry','avvistamento','comunicazione','none') COLLATE utf8mb4_unicode_ci NOT NULL,
...
INSERT INTO `articolo` (`id`, `autore`, `titolo`, `data`, `luogo`, `descrizione`, `contenuto`, `image_path`, `tag`, `featured`, `alt`) VALUES
(1, 1, 'Alla scoperta del Kraken', '2023-05-21 14:04:53', 'Islanda', 'Kraken, il mostro marino della mitologia nordica', 'Nella mitologia nordica, il Kraken era un mostro marino dalle dimensioni gigantesche, in grado di attaccare e distruggere le imbarcazioni per le sue masse enormi. Il Kraken è stato descritto come una creatura dalle tentacoli lunghe e forti, con occhi rossi e un ingente bocca munita di denti affilati.\r\n\r\nMolte sono state le teorie formulate per spiegare origine di questo mostro marino, tra cui ipotesi di una creatura preistorica sopravvissuta alla estinzione dei dinosauri o la descrizione di un calamaro gigante.\r\n\r\nNonostante alcune descrizioni del Kraken siano state esagerate e fantastiche, molte testimonianze parlano di avvistamenti di misteriose creature marine, che potrebbero avere ispirato la figura del Kraken nella mitologia nordica.', 'https://firebasestorage.googleapis.com/v0/b/elusive-f3c33.appspot.com/o/images%2Farticles%2Fartkraken.png?alt=media&token=6fe9f427-b6f8-4e4c-bda5-723ff7a8e372', 'scoperta', 1, 'testo'),
(2, 3, 'Il misterioso Chupacabra', '2023-05-21 14:06:51', 'Ohio', 'Alla scoperta del leggendario animale nordamericano', 'Il chupacabra, leggendario animale che sarebbe responsabile della morte di decine di animali da fattoria, ha attirato attenzione degli scienziati e dei media di tutto il mondo. Ma cosa si sa realmente di questo creatura?\r\n\r\nSecondo le descrizioni degli avvistamenti, il chupacabra avrebbe una pelle simile a quella di un rettile, con spine che si estendono lungo la schiena, occhi rossi e artigli affilati. Molte sono state le teorie formulate per spiegare la presenza di questo strano animale, tra cui ipotesi di una creatura aliena o di un ibrido tra diversi animali.\r\n\r\nTuttavia, per gli scienziati, il chupacabra non ha una base biologica reale e le prove a sostegno della sua esistenza sono poco affidabili. Inoltre, molti degli avvistamenti del chupacabra sono stati attribuiti a attacchi di cani randagi o coyote.\r\n\r\nNonostante ciò, il chupacabra rimane una figura affascinante della cultura popolare latinoamericana e continua ad essere oggetto di attenzione e mistero per molti appassionati di criptozoologia.\r\n', 'https://firebasestorage.googleapis.com/v0/b/elusive-f3c33.appspot.com/o/images%2Farticles%2Fartchupa.jpg?alt=media&token=e83fda13-7187-4c13-a8f6-264b4aa21d22', 'comunicazione', 1, 'testo'),
(3, 3, 'Il Bigfoot delle foreste Nord Americane', '2023-05-21 14:08:58', 'Nevada', 'Sasquatch: alla ricerca della leggendaria creatura del Nord America', 'Sasquatch, noto anche come Bigfoot, è una creatura leggendaria che vive nelle foreste del Nord America, la cui esistenza è stata a lungo discussa tra scienziati e appassionati di criptozoologia.\r\n\r\nSecondo le descrizioni, Sasquatch sarebbe un animale alto fino a 3 metri, che cammina su due zampe e ha un mantello peloso che lo avvolge quasi completamente. Molte sono state le testimonianze di avvistamenti di Sasquatch, principalmente nella zona della Pacific Northwest degli Stati Uniti.\r\n\r\nNonostante le affermazioni di alcuni appassionati che sostengono di avere prove concrete dell esistenza di Sasquatch, gli scienziati non hanno ancora trovato nessuna prova concreta dell esistenza di questa creatura. Tuttavia, il mito di Sasquatch continua ad affascinare molti, alimentando la curiosità e la ricerca di questa leggendaria creatura.\r\n', 'https://firebasestorage.googleapis.com/v0/b/elusive-f3c33.appspot.com/o/images%2Farticles%2Fartbigfoot.png?alt=media&token=37beaef7-a48b-4c8c-8067-f0b10dc6bf10', 'new-entry', 0, 'testo');

-- --------------------------------------------------------

--
-- Table structure for table `articolo_animale`
--
...
--
-- Dumping data for table `articolo_animale`
--

INSERT INTO `articolo_animale` (`articolo`, `animale`) VALUES
(1, 'Kraken'),
(2, 'Chupacapra'),
(3, 'Sasquatch');
...
--
-- Stand-in structure for view `view_articolo_animale`
-- (See below for the actual view)
--
CREATE TABLE `view_articolo_animale` (
`id` int(11)
,`nome` varchar(255)
,`titolo` varchar(255)
,`animale` varchar(100)
...
--
-- Structure for view `view_articolo_animale`
--
DROP TABLE IF EXISTS `view_articolo_animale`;

CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `view_articolo_animale`  AS SELECT `view_articolo_utente`.`id` AS `id`, `view_articolo_utente`.`nome` AS `nome`, `view_articolo_utente`.`titolo` AS `titolo`, `articolo_animale`.`animale` AS `animale` FROM (`view_articolo_utente` join `articolo_animale` on((`view_articolo_utente`.`id` = `articolo_animale`.`articolo`)))  ;
...
--
-- Indexes for table `articolo_animale`
--
ALTER TABLE `articolo_animale`
  ADD PRIMARY KEY (`articolo`,`animale`);
